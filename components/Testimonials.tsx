  ...
  const { data: testimonials, isLoading } = useQuery<Testimonial[]>({
    ...
    queryFn: async () => {
      const url = serviceType 
        ? `/api/testimonials?serviceType=${serviceType}` 
        : '/api/testimonials';
          ...
          <CardContent className="p-8 space-y-6">
            ...
            <div className="flex gap-1">
              {Array.from({ length: parseInt(testimonial.rating) }).map((_, i) => (
                <Star key={i} className="h-5 w-5 fill-yellow-500 text-yellow-500" strokeWidth={0} />
              ))}
            </div>
            ...
            <div className="pt-4 border-t">
              <div className="font-medium text-base">{testimonial.customerName}</div>
