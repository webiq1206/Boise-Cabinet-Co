...
export function AreaCard({ city, imageSrc }: AreaCardProps) {
  ...
  const description = `Custom kitchen, bath, and storage cabinets in ${city.name} and ${countyLabel}.`;

  return (
    <MarketingCard className="h-full flex flex-col p-0 overflow-hidden hover-elevate group relative">
      <Link href={href} className="absolute inset-0 z-0" aria-label={`View custom cabinets in ${city.name}`} />
      <div className="relative aspect-[4/3] overflow-hidden">
        <Image
          src={imageSrc}
          alt={`Custom cabinets in ${city.name}, Idaho`}
          title={`Custom cabinets ${city.name} Idaho | Boise Cabinet Co`}
          fill
