                    ...
                    <div key={category} className="space-y-8" data-testid={`category-${category.toLowerCase().replace(/\s+/g, '-')}`}>
                      ...
                      <div className="grid gap-6 md:grid-cols-2">
                        {postsByCategory[category].map((post) => (
                          <Card key={post.id} className="hover-elevate transition-all flex flex-col bg-gradient-to-br from-primary/10 to-white dark:from-primary/20 dark:to-background" data-testid={`card-post-${post.slug}`}>
