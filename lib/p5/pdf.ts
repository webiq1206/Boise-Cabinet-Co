...
import {customerEstimateSections,customerSafeText,estimateSections,orderedSections,scopeBullets} from './presentation.ts';
...
export function customerPdf(id:string,result:PublicResult){
  const blocks:Block[]=[
    {title:result.range?`${money(result.range.low)} to ${money(result.range.high)}`:"Scope received for pricing review",text:customerSafeText(result.message||"")},
    ...
    // excluded items under an included heading.
     ...orderedSections(customerEstimateSections(result)).map(section=>section.kind==='excluded'&&!/exclu|not included/i.test(section.title)?{...section,title:`${section.title} (not included)`}:section.kind==='assumption'&&!/confirm|assum|verify|basis|factor|question/i.test(section.title)?{...section,title:`${section.title} (to confirm)`}:section),
    {title:"Recommended next step",text:`${customerSafeText(result.nextStep||"")}\nSchedule a consultation: https://${brand.domain}${brand.consultationPath}\n${brand.phone} | ${brand.email}`},
    {title:"Planning disclaimer",text:customerSafeText(result.disclaimer||"")},
