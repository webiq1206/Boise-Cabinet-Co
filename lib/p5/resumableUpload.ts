...
import {verifyUpload,verifyPdfPageLimit,checkOfficeArchiveRanges} from './documents.ts';
...
export async function postUpload(request:Request){
  try{
    ...
    try{
      ...
      const hash=createHash('sha256');let size=0;const pdfParts:Buffer[]=[];
      const stream=Readable.from((async function*(){
        for(let i=0;i<count;i++){
          const data=await readChunk(i);hash.update(data);size+=data.length;if(extension==='pdf')pdfParts.push(data);
          yield data;
        }
      ...
      try{
        ...
        if(extension==='pdf')try{await verifyPdfPageLimit(t.name,Buffer.concat(pdfParts));}
        catch(error){throw new DraftError(error instanceof Error?error.message:'This PDF could not be validated.',422);}
