...
export interface ProcessingStatus {
  phase:'queued'|'preparing'|'instructions'|'reading'|'cross-referencing'|'inventory'|'mapping'|'research'|'normalizing'|'verification'|'retrying';
  message:string;
  updatedAt:string;
  startedAt?:string;
  readPages?:number;
...
export const processingTitles:Record<ProcessingStatus['phase'],string>={
  ...
  mapping:'Matching your scope to the cost book',research:'Researching missing local rates',normalizing:'Formatting verified cost evidence',verification:'Checking scope and pricing coverage',retrying:'Recovering an interrupted step',
};
export function pricingActivity(instructions:string,input:unknown,search:boolean):ProcessingStatus{
  ...
  const phase=search?'research':instructions.startsWith('Inventory')?'inventory':instructions.startsWith('You are a construction estimator')?'mapping':instructions.startsWith('Convert the supplied research')?'normalizing':'verification';
  const message=phase==='inventory'?'Identifying the included work, exclusions and item-level quantities.':phase==='mapping'?(data.repairInstruction?'Resolving findings from the coverage check.':'Matching quantities, materials and labor to your established rates.'):phase==='research'?'Checking published cost evidence for items that need a supported allowance.':phase==='normalizing'?'Formatting the verified source evidence without changing its quantities or cost basis.':'Checking for missing items, duplicate counts, scope restrictions and pricing assumptions.';
