...
import {SCOPE_PLAN_PAGE_LIMIT} from './scope.ts';
...
export async function* analysisSegments(file:AnalysisFile,startPage=0,render:typeof drawingDetails=drawingDetails):AsyncGenerator<AnalysisFile>{
  if(file.type==='application/pdf'){
    ...
    if(!count||count>SCOPE_PLAN_PAGE_LIMIT)throw new Error(`This PDF needs between 1 and ${SCOPE_PLAN_PAGE_LIMIT} pages.`);
