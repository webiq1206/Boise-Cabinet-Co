...
/** Start a separate record from replacement text. The prior draft and uploads remain untouched. */
export function replacementBrowserDraft(current:BrowserDraft):BrowserDraft {
  return {...newBrowserDraft(''),namespace:current.namespace,text:current.text,updatedAt:Date.now(),dirty:true};
}
