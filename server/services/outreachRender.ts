...
function buildOutreachFooter(unsubscribeUrl: string): string {
  ...
  return `
    <div style="padding:24px 30px; background:${EMAIL_BRAND.highlightBg}; border-top:1px solid ${EMAIL_BRAND.border}; text-align:center;">
      <p style="margin:0 0 8px 0; font-size:12px; color:${EMAIL_BRAND.charcoalLight};">${escapeHtml(identity)}</p>
      <p style="margin:0; font-size:12px; color:${EMAIL_BRAND.charcoalLight};">
        You received this because we believe Boise Cabinet Co may be relevant to you.
        <a href="${unsubscribeUrl}" style="color:${EMAIL_BRAND.charcoalLight}; text-decoration:underline;">Unsubscribe</a>.
      </p>
    </div>
  `;
}
