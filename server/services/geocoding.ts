...
async function fetchNominatimSuggestions(input: string): Promise<AddressSuggestion[]> {
  ...
  const res = await fetch(
    ...
    {
      headers: {
        "User-Agent": "BoiseCabinetCo/1.0 (property lookup)",
      },
    }
  );
  if (!res.ok) return [];
...
export function parseFormattedAddress(text: string): PropertyProfileInput {
  ...
  let city = "";
  let state = "ID";
  let streetAddress = parts[0] ?? text;

  if (parts.length >= 2) {
    const cityPart = parts[parts.length - 2] ?? "";
    if (!/^\d{5}/.test(cityPart) && !/county/i.test(cityPart)) {
      city = cityPart.replace(/\s+(ID|Idaho)$/i, "").trim();
    }
    const last = parts[parts.length - 1] ?? "";
    if (/^(ID|Idaho)$/i.test(last) || /^\d{5}/.test(last)) {
      if (parts.length >= 3) {
        city = parts[parts.length - 2].replace(/\s+(ID|Idaho)$/i, "").trim();
      }
    }
  }
