...
function row(label: string, value: string): string {
  return `<tr><td style="padding:6px 10px 6px 0;color:${EMAIL_BRAND.charcoalLight};font-size:13px;vertical-align:top;white-space:nowrap;">${escapeHtml(label)}</td><td style="padding:6px 0;font-size:13px;color:${EMAIL_BRAND.charcoal};">${value}</td></tr>`;
}

function selectionsTable(session: EstimatorSession): string {
  const entries = Object.entries(session.selections ?? {});
  if (!entries.length) return `<p style="font-size:13px;color:${EMAIL_BRAND.charcoalLight};">No selections recorded.</p>`;
...
function contactBlock(session: EstimatorSession): string {
  if (!session.contactPhone && !session.contactEmail && !session.contactName) {
    return `<p style="margin:0;font-size:13px;color:${EMAIL_BRAND.charcoal};"><strong>No contact information was provided.</strong> This is an anonymous partial completion; it tells you where the flow lost someone, not who.</p>`;
  }
  ...
  return `<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
    ...
    ${session.contactPhone ? row("Phone", `<a href="${tel}" style="color:${EMAIL_BRAND.charcoal};">${escapeHtml(pretty)}</a>`) : ""}
...
function sessionFacts(session: EstimatorSession): string {
  ...
  return `<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
    ...
    ${row("Page", `<a href="${SITE_BASE_URL}${escapeHtml(session.pagePath)}" style="color:${EMAIL_BRAND.charcoal};">${escapeHtml(session.pagePath)}</a>`)}
...
export function buildAbandonmentEmail(session: EstimatorSession): { subject: string; html: string } {
  ...
  const content = `
    ...
    <p style="margin:22px 0 0;font-size:12px;color:${EMAIL_BRAND.charcoalLight};">A click to call or text shows intent only; it does not confirm a conversation happened. Exit method is reported as unknown when the browser gave no signal.</p>`;
