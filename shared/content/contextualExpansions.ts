...
const BASE_TOPICS = [
  ...
  'cabinet design and installation keeps design, permits, and construction under one contract, reducing gaps that cause change orders mid-project.',
  ...
  'Comparing cabinet project cost to moving should include commissions, moving costs, and emotional disruption, not just sticker price.',
  ...
  'Meridian subdivisions often benefit from open kitchen conversions and mudbuilt-in storages at rear entries.',
];
...
const HUB_SNIPPETS: Record<string, string[]> = {
  ...
  'choosing-cabinet-company': [
    'Interview at least two cabinet design and installation firms with aligned scope before you sign.',
    'Ask who owns communication during installation, not only who sells the project.',
  ],
  ...
  'cabinet-roi': [
    'Pre-sale cabinet upgrades should target buyer expectations in your specific subdivision.',
...
export function buildContextualExpansions(slug: string, hubSlug: string, count: number): string {
  ...
  for (let i = 0; i < count; i++) {
    ...
    html += `<div class="summary-block"><p>${line} ${extra} <a href="/guides/treasure-valley-cabinet-guide">Treasure Valley</a> · <a href="/contact">Consultation</a>.</p></div>`;
  }
  return html;
}
