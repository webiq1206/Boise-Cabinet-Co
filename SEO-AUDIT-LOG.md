...
| 8 | MEDIUM | `/404` | Custom not-found page had no `metadata` export — search engines saw an empty `<title>` and no robots directive. | Added `metadata` with title, description, and `robots: { index:false, follow:true }`. | `app/not-found.tsx` |
...
| 10 | LOW | Site-wide | Default OG image is the brand logo (`/images/lawn-care-kuna-logo.png`). Declared 1200×630 but the asset is the square logo — likely auto-cropped by social platforms. | LOGGED, not auto-fixable in this task: requires a designer-produced 1200×630 OG card. Width/height already declared so social cards still render; lower CTR than a dedicated card. Recommend follow-up: add `public/og-image.jpg` and update `app/layout.tsx`. |
