...
for(const width of [320,390,430,600,768,1024,1366,1440,1920]){
 ...
 try{
  ...
  const bar=page.locator('div.fixed.left-0.right-0.bottom-0').filter({visible:true});
  if(width<1280 && await bar.count())throw new Error('Sticky action bar overlaps active catalog search');
  await page.screenshot({path:`p5-verification/search-found-${width}.jpg`});
  ...
  await page.screenshot({path:`p5-verification/search-empty-${width}.jpg`});
 ...
 await page.screenshot({path:`p5-verification/catalog-${width}.jpg`,fullPage:true});await page.close();await fs.writeFile('p5-verification/catalog-results.json',JSON.stringify(results,null,2));
}
