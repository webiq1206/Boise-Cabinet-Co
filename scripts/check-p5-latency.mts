...
const optionQuestion=questions.find(question=>question.instructionId&&question.values?.some(value=>/painted MDF\/wood.*4 hours/i.test(value)));
if(!files.length&&optionQuestion){
 const selected=applyAlternativeSelection(extraction,merged.answers,optionQuestion.reason,'painted MDF/wood');
