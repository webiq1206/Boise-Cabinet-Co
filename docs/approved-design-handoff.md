...
The catalog PDF.js worker is now synchronized from the installed package before both development and production builds. This prevents API/worker version mismatches; the existing Replit build script already performs the same synchronization.
