...
- **Server-Side Lead Pricing**: Quote API (`app/api/quotes/route.ts`) calculates per-service prices from measurement data (property size, linear feet, zones, etc.) using SERVICE_PRICING_RATES, rather than relying on client-sent totals. Lead prices are always 10% of the total per-visit estimate (midpoint of quote range), with a $15 minimum and $50 maximum, rounded to nearest $5. Daily price decay of 1.5%/day with 20% floor.
