...
- **Lead Distribution System**: A B2B lead marketplace with admin dashboard, subcontractor portal, privacy protection, automated lead pricing, legal agreement flow, and in-app notifications. Integrates with Stripe for payments. Supports account credits system: admins can add credits to subcontractor accounts, which are applied automatically during lead purchases. Three purchase scenarios: full credit (no Stripe charge), partial credit (Stripe charges remainder), and full Stripe (no credits). Credit transactions are tracked in `credit_transactions` table for audit trail. `leadPurchases` records `creditsUsed` per purchase. Lead cards are collapsible on mobile with key info visible at a glance. Admins can permanently delete leads via confirmation dialog. Admin recovery endpoint (`/api/admin/resolve-payment`) supports both POST (with leadId/userId body) and GET (auto-resolves known unresolved purchases) for fixing failed purchase confirmations where Stripe charged but the backend did not record the purchase. Next.js instrumentation hook (`instrumentation.ts`) runs on production startup to auto-resolve known broken purchases and auto-archive stale leads. Purchased leads in the "My Purchases" tab reveal full contact info (name, phone, email, address) with clickable tel/mailto links, hide purchase controls/checkbox/watchlist, and show a "Purchased" badge with purchase date. Lead purchase exclusivity: purchased leads are completely hidden from all other subcontractors (not just masked); only the buyer sees them in My Purchases and admin sees everything. Auto-archival: leads older than 7 days that remain unpurchased are automatically set to "archived" status and removed from subcontractor views. Auto-archive runs on production startup (instrumentation.ts) and inline on every leads API GET request. Admin dashboard has a dedicated "Archived" tab to view archived leads. Lead statuses: pending_admin, accepted, available, purchased, declined_admin, archived.
