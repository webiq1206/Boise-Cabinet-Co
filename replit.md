...
## Recent Changes
- **2026-02-07**: Migrated all missing API endpoints from Express to Next.js API routes for both portals. Created 12 new Next.js API routes: /api/leads (GET with filters/masking), /api/leads/[leadId] (PATCH admin update), /api/leads/[leadId]/accept (admin), /api/leads/[leadId]/decline (admin with subcontractor notifications), /api/leads/[leadId]/notes (admin), /api/leads/watchlist (subcontractor), /api/leads/[leadId]/watch and /unwatch (subcontractor), /api/leads/[leadId]/purchase (with Stripe verification), /api/leads/bulk-purchase, /api/leads/purchases (history), /api/user/notification-preferences. All routes enforce proper role-based access control (admin vs subcontractor). Replaced all emojis in subcontractor portal with lucide-react icons.
- **2026-02-07**: Fixed critical login redirect bug - all auth routes (login, callback, post-login, logout) were redirecting to localhost:5000 instead of production domain. Root cause: `new URL("/path", request.url)` resolves to localhost inside Replit's proxy infrastructure. Fix: Added `getExternalUrl()` helper using `x-forwarded-host`/`x-forwarded-proto` headers to build correct external URLs. Also fixed subcontractor login using wrong query param (`?redirect=` → `?returnTo=`). Added `redirect_uri` to OIDC token exchange for proper callback validation.
- **2026-02-04**: Mobile performance optimizations based on Lighthouse audit. Converted hero image from 98KB PNG to 9KB WebP (90% reduction). Added preload link with fetchPriority="high" for faster LCP. Fixed color contrast issue in footer (text-primary→text-foreground). Fixed 3 non-descriptive "Learn More" links to use descriptive text like "Explore Lawn Care Services". Expected improvement from 82% to 85-90% performance score.
- **2026-02-04**: Implemented complete Next.js authentication system using Replit OIDC. Created lib/auth.ts with iron-session for secure cookie-based sessions and PKCE for OIDC. Added Next.js API routes: /api/auth/user, /api/login, /api/callback, /api/post-login, /api/logout. Fixed payment intent routes to use server-side session auth instead of trusting client-provided userId. Auth system enforces SESSION_SECRET, validates session expiration, and uses proper protocol detection from request URLs. Role-based redirects after login (subcontractor→portal, admin→dashboard).
- **2026-01-22**: Fixed broken sitemap URLs and 301 redirect issues. Added '/commercial' to valid route patterns in server/redirects.ts. Fixed sendLeadPurchasedNotification and sendLeadPurchaseConfirmation function call signatures in server/routes.ts. Validated all 313 sitemap URLs return 200 status. Updated email templates with subtle green-to-white gradient headers for consistent branding.
- **2026-01-20**: Fixed admin login access control for non-admin users. Added cookie-based `auth_returnTo` persistence across OAuth redirects. Updated SubcontractorRouteEnforcer to exclude /admin routes from automatic redirects. Non-admin users attempting admin login now correctly see "does not have admin access" error.
- **2026-01-20**: Improved SimpleQuoteWizard with better error handling and UX. Added toast notifications for submission success/error feedback. Added "Use this address" button for manual address lookup when autocomplete suggestions aren't available. Implemented auto-advance to Phase 2 after both successful and fallback property lookups. Enhanced serviceData payload to include measurements for ALL selected services (not just primary). Fixed payload structure to properly match server quoteSubmissionSchema (serviceType, propertyType, etc.).
- **2025-12-01**: Massive blog content expansion with 50 new SEO-optimized articles (total now 92). New posts organized in 5 categories: Seasonal & Lawn Maintenance (10), Irrigation & Advanced Care (10), Landscaping Design & Ideas (10), Homeowner Guides & Specialty (10), and Property Types & Problem Solving (10). Each post 1500-2000 words with H2/H3 hierarchy, 6-10 internal service links, 5-6 FAQs, and local Idaho/Treasure Valley focus. Updated sitemap.xml (306 URLs), llms.txt, and replit.md. All posts properly integrated into shared/blogContent.ts.
